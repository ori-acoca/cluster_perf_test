...
def check_connection(nodes: list, user, key):
    ...
    for node in nodes:
        ...
        if uptime:
            ...
            print(f"Connection to {node} failed - Skipping")
...
def validate_nic_type(node, link_layer, user, key):
    ...
    :return:
    """
    actual_link_layer = execute_on_remote_nodes(host=node,
                                                command=f"ip route show | grep {node} | awk '{{print $3}}' | xargs ip link show | grep link | awk '{{print $1}}'",
                                                user=user, key=key, suppress_output=False)
    if link_layer not in actual_link_layer:
        logger.error(f"NIC with IP {node} is {actual_link_layer}, {link_layer} was expected")
        return False
    logger.info(f"Actual link layer on {node} is {actual_link_layer} - Passed link_layer validation")
    return link_layer in actual_link_layer
...
def cleanup_leftovers(nodes: list, user, key, supress_notice=True):
    ...
    for node in nodes:
        if not supress_notice:
            ...
            execute_on_remote_nodes(node, "lsof -n -i :18515 | awk '{print $2}' | grep -v PID | xargs kill -9", user, key,
                                    suppress_output=True)
            execute_on_remote_nodes(node, "lsof -n -i :5001 | awk '{print $2}' | grep -v PID | xargs kill -9", user, key,
                                    suppress_output=True)
...
def does_ib_send_bw_installed(node, user, key):
    ...
    return "LISTEN" in execute_on_remote_nodes(node, "ss -tulpn | grep ib_send_bw", user, key, suppress_output=False)
...
def does_ib_send_lat_installed(node, user, key):
    ...
    return "LISTEN" in execute_on_remote_nodes(node, "ss -tulpn | grep ib_send_lat", user, key, suppress_output=False)
